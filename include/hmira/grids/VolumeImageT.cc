...
bool
VolumeImageT<Scalar>::read( const char* _fname )
{
    // get file infos
    ...
    if ( suffix == "hdr" )
    {
        ...
        if ( header.open( QIODevice::ReadOnly ) )
        {
            ...
            if ( buf32 != 348 )
            {
                std::cout << " ERROR: wrong file length: " << header_name.toAscii().data() << ": " << buf32 << std::endl;
                return false;
            }
        ...
        {
            std::cout << "ERROR: reading header file: " << header_name.toAscii().data() << std::endl;
            return false;
        }
    }
    else if ( suffix == "dat" )
    {
        std::ifstream header( header_name.toAscii().data(), std::ios::in );
        if ( !header.is_open() )
        {
    ...
    std::ifstream datafile( rawfile_name.toAscii().data(), std::ios::binary );
    if ( !datafile.is_open() )
    {
        std::cout << "ERROR: reading data file: " << rawfile_name.toAscii().data() << std::endl;
        return false;
    }
    int x = 0;
    int y = 0;
...
template <class Scalar>
bool
VolumeImageT<Scalar>::write( const char* _fname, VolumeImageMode _mode )
{
...
template <class Scalar>
void
VolumeImageT<Scalar>::update_min_max()
{
...
template <class Scalar>
void
VolumeImageT<Scalar>::getByteData( std::vector<unsigned char> &_result ) const
{
...
template <class Scalar>
void
VolumeImageT<Scalar>::getData( std::vector<float> &_result ) const
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
...
template <class Scalar>
void
VolumeImageT<Scalar>::i_to_xyz( int _i, int &_x, int &_y, int &_z )
{
...
template <class Scalar>
void
VolumeImageT<Scalar>::get_neighbors_in_r( const Grid::PointIdx &_p,
        const int _r,
...
template<class Scalar>
void
VolumeImageT<Scalar>::bilateral_filter( Scalar _hs, Scalar _hr,  int _iterations )
{
    QTime stop_watch;
    stop_watch.start();
...
template<class Scalar>
void
VolumeImageT<Scalar>::bilateral_update( std::vector<Scalar> &_src,
...
template<class Scalar>
void
VolumeImageT<Scalar>::bilateral_filter_simple( Scalar _sigma_s, Scalar _sigma_r,  int _iterations )
{
    QTime stop_watch;
    stop_watch.start();
...
template<class Scalar>
void
VolumeImageT<Scalar>::bilateral_update_simple( std::vector<Scalar> &_src,
        std::vector<Scalar> &_dst,
...
template<class Scalar>
void
VolumeImageT<Scalar>::set_border()
{
    unsigned int x;
    unsigned int y;
    unsigned int z;
...
template<class Scalar>
void
VolumeImageT<Scalar>::set_bandwidth( Scalar _hs, Scalar _hr )
{
...
template<class Scalar>
void
VolumeImageT<Scalar>::init( const Vec3&  _origin,
                            const Vec3&  _x_axis,
                            const Vec3&  _y_axis,
                            const Vec3&  _z_axis,
                            unsigned int            _x_res,
...
template<class Scalar>
void
VolumeImageT<Scalar>::getByteDataGradient( std::vector<unsigned char> &_result ) const
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
...
template <class Scalar>
void
VolumeImageT<Scalar>::getDataGradient( std::vector<float> &_result ) const
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
...
template<class Scalar>
typename VolumeImageT<Scalar>::Vec3
VolumeImageT<Scalar>::calcGradient( const unsigned int &_x,
...
template<class Scalar>
void
VolumeImageT<Scalar>::assign_gradients()
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
...
template<class Scalar>
void
VolumeImageT<Scalar>::normalize_gradients()
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
...
template<class Scalar>
void
VolumeImageT<Scalar>::histogram( const int _n_bins, std::vector<unsigned int> &_bins, Scalar &_size, bool _to_image )
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
...
template<class Scalar>
void
VolumeImageT<Scalar>::normalize_with_histogram( Scalar _max, Scalar _percent, int _n_bins )
{
    unsigned int x_res( this->x_resolution() );
    unsigned int y_res( this->y_resolution() );
    unsigned int z_res( this->z_resolution() );
